...
export function PartnerForm() {
  ...
  useEffect(() => {
    void getHosts().then(setHosts).catch(() => setHosts([]));
  }, []);
    ...
    <form className="cartoon-section space-y-6 bg-[#f0fdf8] p-8" onSubmit={submit}>
      ...
      <fieldset className="space-y-4">
        ...
        <div>
          ...
          <input className="cartoon-input w-full bg-white p-3" id="partner-address" name="address" placeholder="e.g. 42 Cat Street" required />
        </div>
        <div className="grid gap-4 sm:grid-cols-3">
          <div>
            <label className="mb-1 block text-sm font-bold text-stone-700" htmlFor="partner-cep">Postal code</label>
            <input className="cartoon-input w-full bg-white p-3" id="partner-cep" maxLength={9} name="cep" placeholder="e.g. 00000-000" required />
          </div>
          <div>
            <label className="mb-1 block text-sm font-bold text-stone-700" htmlFor="partner-city">City</label>
            <input className="cartoon-input w-full bg-white p-3" id="partner-city" name="city" placeholder="e.g. Springfield" required />
          </div>
          <div>
            <label className="mb-1 block text-sm font-bold text-stone-700" htmlFor="partner-state">State</label>
            <input className="cartoon-input w-full bg-white p-3" id="partner-state" maxLength={2} name="state" placeholder="e.g. NY" required />
          </div>
        </div>
      </fieldset>
      ...
      <fieldset className="space-y-4">
        <legend className="mb-2 text-lg font-extrabold tracking-wide text-teal-900">Your offer</legend>
        <div>
          <label className="mb-1 block text-sm font-bold text-stone-700" htmlFor="partner-host">Host</label>
          <select className="cartoon-input w-full bg-white p-3" id="partner-host" name="host_id" required>
        ...
        <div>
          <label className="mb-1 block text-sm font-bold text-stone-700" htmlFor="partner-amount">Proposed amount ($)</label>
          <input className="cartoon-input w-full bg-white p-3" id="partner-amount" min="1" name="proposed_amount" placeholder="e.g. 200" required step="0.01" type="number" />
        </div>
        <div>
          <label className="mb-1 block text-sm font-bold text-stone-700" htmlFor="partner-message">Message</label>
          <textarea className="cartoon-input w-full bg-white p-3" id="partner-message" name="proposed_message" placeholder="Tell the host why you want to purrtner up." rows={3} />
        </div>
      </fieldset>
      ...
      <fieldset className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-1 block text-sm font-bold text-stone-700" htmlFor="partner-email">Email</label>
            <input className="cartoon-input w-full bg-white p-3" id="partner-email" name="contact_email" placeholder="you@business.com" required type="email" />
          </div>
          <div>
            <label className="mb-1 block text-sm font-bold text-stone-700" htmlFor="partner-phone">Phone</label>
            <input className="cartoon-input w-full bg-white p-3" id="partner-phone" name="contact_phone" placeholder="(555) 000-0000" required />
          </div>
        </div>
      </fieldset>
