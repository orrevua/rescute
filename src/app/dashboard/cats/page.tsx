            ...
            {cats.map((cat) => (
              <article className="cartoon-card bg-white p-5" key={cat.id}>
                ...
                <div className="mt-4 flex gap-3">
                  <Link
                    className="cursor-pointer text-sm font-bold text-teal-800 hover:underline"
      ...
      {deleting && (
        <Modal title="Remove cat" onClose={() => setDeleting(null)}>
          <p className="text-stone-700">
            Are you sure you want to remove <span className="font-bold">{deleting.name}</span>? This
